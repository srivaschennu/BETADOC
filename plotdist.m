function plotdist(basename,measure,bandidx,varargin)
...
nchan = size(matrix,2);
chanpairs = zeros(((nchan*nchan)-nchan)/2,2);
cpidx = 0;
for chann1 = 1:nchan
    for chann2 = 1:nchan
        if chann1 < chann2
...
elseif strcmp(measure,'participation coefficient')
    d = mahal(squeeze(mean(graphdata{m,weiorbin}(bandidx,trange,:),2))', ...
        squeeze(mean(graph{m,weiorbin}(selpatidx,bandidx,trange,:),3)));
...
elseif strcmp(measure,'median')
    d = pdist2(squeeze(allcoh(selpatidx,bandidx,sub2ind([nchan nchan],chanpairs(:,1),chanpairs(:,2)))),...
        squeeze(allcoh(selpatidx,bandidx,sub2ind([nchan nchan],chanpairs(:,1),chanpairs(:,2)))));
    d = pdist2(matrix(bandidx,sub2ind([nchan nchan],chanpairs(:,1),chanpairs(:,2))),...
        squeeze(allcoh(selpatidx,bandidx,sub2ind([nchan nchan],chanpairs(:,1),chanpairs(:,2)))));
