...
export_fig(sprintf('figures/auc_%s_%s.tiff',listname,param.group),'-r200','-p0.01');
close(gcf);
...
if strcmp(param.plotcm,'on')
    ...
    for g = 1:size(stats,2)
        ...
        export_fig(gcf,sprintf('figures/clsyfyr_%s_%s_%s_vs_%s_cm.tiff',listname,param.group,param.groupnames{grouppairs(g,1)+1},param.groupnames{grouppairs(g,2)+1}),'-p0.01');
        close(gcf);
    end
end